...
const int MAX_N=5000001,MAX_NLINE=50001<<4,MAX_NN=50001;
int S[MAX_N],Lc[MAX_N],Rc[MAX_N],h[MAX_N];
data T[MAX_N];
int root[MAX_NLINE],a[MAX_NN];
map<int,int> MM;
set<int> ST[MAX_N];
...
inline void change(int i,int dat){
	int pi=PRE(i),ni=NXT(i),v=a[i];data rett;
	...
	if (ni<=n){
		int PNI=PRE(ni);
		for (int l=1,r=n,x=1;;){
			rett=Delete(root[x],mp(a[ni],i,0));
	...
	if (nn<=n){
		int PN=PRE(i);
		for (int l=1,r=n,x=1;;){
			rett=Delete(root[x],mp(a[nn],PN,0));
