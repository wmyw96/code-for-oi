...
我们看，如果我们假设选择了[1][4]的话，这里要满足[3]的流量要比[4]大，[2]的流量要比[1]大，[1]满足不了[2]，[4]的流量对于[3]来说太少了囧、（不然的话就选两个最小的。）
...
对于A、B，如果两者没关系，就会分别取小的（可以想象一下就是2个割）

然而，我们再次思考，如果两者有关系，那么如果两个东西要分开来做的话，最小割中一定要包含容量为extra的这个。

我们继续考虑，如果我们取了A的way1，B的way2，就就必须要取（A，B），否则还是s仍然可以从s走到t，如果不取(A,B)的话，那就必须要取（way1，way1），或者（way2，way2），这样才能不连通。
